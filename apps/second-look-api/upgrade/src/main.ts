/**
 * This is not a production server yet!
 * This is only a minimal backend to get started.
 */

import { Logger } from '@nestjs/common';
import { NestFactory } from '@nestjs/core';
import { DocumentBuilder, SwaggerModule } from '@nestjs/swagger';
import { StoplightElementsModule } from 'nestjs-stoplight-elements';

import { AppModule } from './app/app.module';

async function bootstrap() {
  const app = await NestFactory.create(AppModule);

  const config = new DocumentBuilder()
    .setTitle('Upgrade - Second Look API')
    .setDescription(
      `Lorem ipsum Bug Catcher's favorite Pokémon is Mr. Mime. Waiter used a Luxury Ball to catch Dragonair. Porygon learned Conversion. Clefable is a Fairy Pokémon.

Lorem ipsum Burglar spotted Jynx in Hoenn. You can find the Cinnabar Gym in Kanto. Vileplume used Petal Dance. You can find the Mossdeep Gym in Hoenn. Squirtle learned Water Gun.`
    )
    .setBasePath('/api')
    .setVersion('1.0')
    .addTag('upgrade')
    .build();

  const document = SwaggerModule.createDocument(app, config, {
    // linkNameFactory: 
  });

  const StoplightElements = new StoplightElementsModule(app, document, {
    router: 'hash',
    layout: 'sidebar',
    logo: 'data:image/svg+xml;base64,',
  });

  
  await StoplightElements.start('/docs');

  const port = process.env.PORT || 3000;
  await app.listen(port);
  Logger.log(`🚀 Application is running on: http://localhost:${port}`);
}

bootstrap();
